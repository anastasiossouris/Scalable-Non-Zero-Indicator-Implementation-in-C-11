...
int main(void){
	...
	out_file.open("snzi-no-contention.dat");
...
void run_experiment_for_tree(std::size_t K, std::size_t H, std::vector<double>& all_visits){
	...
	auto thread_job = [](concurrent::no_contention_handling_snzi& snzi_object, int id, std::atomic<bool>& flag, unsigned long& visits){
	...
	for (std::size_t i = 0; i < num_threads_count; ++i){
		...
		concurrent::no_contention_handling_snzi snzi_object(K,H, how_many_threads); // the snzi for this experiement
		std::cout << "Done" << std::endl;
